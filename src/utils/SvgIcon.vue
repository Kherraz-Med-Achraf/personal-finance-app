<template>
    <div class="svg-icon">
      ...
      <template v-else-if="name === 'maximize'">
        <!-- Un autre SVG pour maximiser -->
        <svg fill="none" height="20" viewBox="0 0 20 20" width="20" xmlns="http://www.w3.org/2000/svg">
          <!-- SVG path for maximize -->
        </svg>
      </template>
      ...
      <template v-else>
        <svg fill="none" height="20" viewBox="0 0 20 20" width="20" xmlns="http://www.w3.org/2000/svg">
          <!-- Un SVG par défaut -->
          <path
            d="M10 0C4.477 0 0 4.477 0 10s4.477 10 10 10 10-4.477 10-10S15.523 0 10 0zm0 18C5.589 18 2 14.411 2 10S5.589 2 10 2s8 3.589 8 8-3.589 8-8 8z"
          />
        </svg>
      </template>
    </div>
  ...
  .svg-icon {
    ...
    path {
      fill: red; // Couleur par défaut pour les SVG
      transition: fill 0.3s ease;
    }
  
    &:hover path {
      fill: grey; // Couleur au survol
    }
  }
  </style>
